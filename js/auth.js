...
(()=>{
  ...
  document.addEventListener('DOMContentLoaded',async()=>{if(!publicPage){await protect();await fillUser()}document.querySelectorAll('[data-logout]').forEach(b=>b.addEventListener('click',logout))});
  window.Auth={currentUser,protect,logout,fillUser};
})();
